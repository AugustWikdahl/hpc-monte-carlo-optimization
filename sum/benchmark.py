...
# --- Setup Output Paths (Internal Project Structure) ---
FOLDER_DIR = os.path.dirname(os.path.abspath(__file__)) 
PROJECT_ROOT = os.path.dirname(FOLDER_DIR) 
RESULTS_DIR = os.path.join(PROJECT_ROOT, "logs", "sum")
...
def run_experiment(variable_name, scenarios, m_val, i_val):
    print(f"\n>> Benchmarking {variable_name} Scaling (Summation Trick)...")
...
ax1.errorbar(df_i["Value"], df_i["Avg_Time"], yerr=df_i["Std_Dev"], fmt='-o', color='orange')
...
ax2.errorbar(df_m["Value"], df_m["Avg_Time"], yerr=df_m["Std_Dev"], fmt='-s', color='darkorange')
